...
TODO: what are the exact problems we are trying to solve, why are current tools insufficient, etc.
...
Given the high financial stakes in this environment, it seems clear that allowing reentrant calls by default is not ideal. Most projects do not use reentrancy at all, and thus developers should not be forced to reason about such cases unless they intend to specifically make use of reentrant calls. This does mean that the language no longer has a one-to-one correspondence with the behaviour of the EVM itself. However, this allows inexperienced developers to simcply reason about their own code without needing to worry about the intricacies of the underlying virtual machine. \\
...
\\
TODO: probably good to insert a figure clarifying the three different approaches mentioned: labelling entrypoints, labelling calls, our solution (i.e. restricting to single call path)
